...
const deleteFlat = async (req, res, next) => {
  try {
    ...
    if (req.user.id !== flat.listedBy) {
      return next(
        errorHandler(401, "You can only delete your own flat or PG listing !")
      );
    }
...
const updateFlat = async (req, res, next) => {
  try {
    ...
    if (req.user.id !== flat.listedBy) {
      return next(
        errorHandler(401, "You can only update your own flat or PG listing")
      );
    }

    const updatedFlat = await flats.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
...
const getFlats = async (req, res, next) => {
  try {
    ...
    const totalListings = await flats.countDocuments(queryFilters);

    const fetchedFlats = await flats
      .find(queryFilters)
      .sort({ [sort]: order })
      .limit(limit)
      .skip(startIndex);
