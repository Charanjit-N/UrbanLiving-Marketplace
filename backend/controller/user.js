...
const userLogin = async (req, res,next) => {
    ...
    try{
        ...
        if (user && await bcrypt.compare(password, user.password)) {
            ...
            res.cookie('access_token',token,{httpOnly:true}).status(200).json(remain);
        }
        else {
...
const updateUser = async (req,res,next)=>{
    ...
    if(req.user.id !== id) return next(errorHandler(401,"You can update only your own account !!"))
...
const deleteUser = async (req,res,next)=>{
    ...
    try{
        ...
        res.clearCookie('access_token');
        res.status(200).json('user has been deleted and his listed flats also deleted!');
    }
    catch(err){
        next(err);
    }
...
module.exports = {userSignUp, userLogin, userLogout,getUser,deleteUser,updateUser, getFlatsListedByUser}