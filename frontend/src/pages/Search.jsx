...
export default function Search() {
  ...
  useEffect(() => {
    ...
    const fetchListings = async () => {
      ...
      if (data.listings.length < data.total) {
        setShowMore(true);
      } else {
        setShowMore(false);
      }
  ...
  const handleChange = (e) => {
    if (
      e.target.id === "all-category" ||
      e.target.id === "apartment" ||
      e.target.id === "pg"
    ) {
      setSideformdata({ ...sideformdata, category: e.target.id });
    }

    if (
      e.target.id === "all-type" ||
      e.target.id === "rent" ||
      e.target.id === "sale"
    ) {
      setSideformdata({ ...sideformdata, type: e.target.id });
    }
  ...
  const onShowMoreClick = async () => {
    ...
    if (newListings.length >= data.total) {
      setShowMore(false);
    }
  };
  ...
  return (
    <div className="flex flex-col md:flex-row">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-8">
          ...
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <div className="flex gap-2">
              <input
                checked={sideformdata.category === "apartment"}
                onChange={handleChange}
                type="checkbox"
                id="apartment"
                className="w-5"
            ...
            <div className="flex gap-2">
              <input
                checked={sideformdata.category === "pg"}
                onChange={handleChange}
                type="checkbox"
                id="pg"
                className="w-5"
            ...
            <div className="flex gap-2">
              <input
                checked={sideformdata.category === "all-category"}
                onChange={handleChange}
                type="checkbox"
                id="all-category"
                className="w-5"
          ...
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <div className="flex gap-2">
              <input
                checked={sideformdata.type === "sale"}
                onChange={handleChange}
                type="checkbox"
                id="sale"
                className="w-5"
            ...
            <div className="flex gap-2">
              <input
                checked={sideformdata.type === "all-type"}
                onChange={handleChange}
                type="checkbox"
                id="all-type"
                className="w-5"
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="border bg-white rounded-lg p-2 cursor-pointer"
              id="sort_order"
            >
          ...
          <button className="bg-yellow-300 p-3 rounded-lg  hover:opacity-95 font-semibold cursor-pointer">
            Search
          </button>
        </form>
      </div>
      ...
      <div className="flex-1">
        <h1 className="text-2xl font-semibold border-b border-gray-300 p-2  mt-5">
          Search Results:
        </h1>
        <div className="p-7 flex flex-wrap gap-4">
          ...
          {showMore && (
            <button
              ...
              className="text-blue-800 hover:underline p-7 text-center w-full cursor-pointer"
            >
              Show more
            </button>
          )}
